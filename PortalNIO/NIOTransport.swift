...
public struct NIOTransport: HTTPTransport {
  ...
  // Injectable for testing; defaults to HTTPClient.shared execution.
  var httpExecutor: (HTTPClientRequest) async throws -> (Data, Int, [Header])
  ...
  public init(cache: (any PortalCache)? = nil) {
    ...
    self.httpExecutor = NIOTransport.defaultExecutor
  }

  init(cache: (any PortalCache)? = nil, executor: @escaping (HTTPClientRequest) async throws -> (Data, Int, [Header])) {
    self.cacheHandler = cache.map(CacheHandler.init)
...
private extension NIOTransport {
  static func defaultExecutor(_ httpRequest: HTTPClientRequest) async throws -> (Data, Int, [Header]) {
    ...
    let headers: [Header] = response.headers.map {
      Header(key: HeaderKey(stringLiteral: $0.name), value: HeaderValue(stringLiteral: $0.value))
    }
  ...
  func performRequestWithHeaders(_ request: PortalRequest) async throws -> (Data, Int, [Header]) {
    ...
    return try await httpExecutor(httpRequest)
  }
